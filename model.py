...
from torch.nn import Module, Conv1d, BatchNorm1d, Dropout, Parameter
...
class FocalModulation1d(nn.Module):
    """
    把 FocalNet 的 2-D 调制改成 1-D 时序版本
    输入: (B, C, T)
    """
    def __init__(self, dim, focal_kernel=7, proj_drop=0.):
        super().__init__()
        self.dim = dim
        self.focal_kernel = focal_kernel
        pad = focal_kernel // 2

        # 1) 局部分支：深度卷积 + LN
        self.focal_dw = nn.Conv1d(dim, dim, kernel_size=focal_kernel,
        ...
        self.act = nn.GELU()
        self.proj_out = nn.Conv1d(2 * dim, dim, 1)
    ...
    def forward(self, x):
        ...
        # 局部分支
        focal = self.focal_dw(x)          # (B, C, T)
        focal = focal.transpose(1, 2)     # (B, T, C)
        focal = self.norm(focal)
        focal = focal.transpose(1, 2)     # (B, C, T)

        # 门控
        x = self.proj_in(x)               # (B, 2C, T)
        x = self.act(x)
...
class FocalModulationBlock(nn.Module):
    def __init__(self, d_model, kernel_size=7, dropout=0.1):
        ...
        self.focal = FocalModulation1d(d_model, focal_kernel=kernel_size, proj_drop=dropout)
    ...
    def forward(self, x):
        # x: (B, C, T)
        x = x + self.focal(self.norm1(x.transpose(1, 2)).transpose(1, 2))
        x = x + self.ffn(self.norm2(x.transpose(1, 2)).transpose(1, 2))
        return x
...
class SingleStage(nn.Module):
    def __init__(self, num_layers, num_f_maps, dim, num_classes, dropout):
        ...
        self.layers = nn.ModuleList([
            FocalModulationBlock(num_f_maps, kernel_size=7, dropout=dropout)
...
class MyTransformer(nn.Module):
    def __init__(self, num_decoders, num_layers, r1, r2, num_f_maps, dim, num_classes, dropout, arch_type, pos_enc):
        ...
        # 编码器
        self.encoder = SingleStage(num_layers, num_f_maps, num_f_maps, num_classes, dropout)

        # 解码器
        self.decoders = nn.ModuleList([
            SingleStage(num_layers, num_f_maps, num_f_maps, num_classes, dropout)
    ...
    def forward(self, x, mask):
        ...
        for decoder in self.decoders:
            out, enc_feature = decoder(F.softmax(out, dim=1) * mask[:, 0:1, :],
                                       enc_feature * mask[:, 0:1, :], mask)
            outputs.append(out.unsqueeze(0))
