...
elif args.dataset == 'finebio':
    lr = 0.001
    bz = 4

data_root = './data/' + args.dataset
features_path = os.path.join(data_root, 'features', args.feature)
if args.feature.startswith('bridge'):
    features_path += '_' + args.split
gt_path        = os.path.join(data_root, 'groundTruth')
mapping_file   = os.path.join(data_root, 'mapping.txt')

vid_list_file     = os.path.join(data_root, 'splits', f'train.split{args.split}.bundle')
vid_list_file_tst = os.path.join(data_root, 'splits', f'test.split{args.split}.bundle')

model_dir   = os.path.join(args.model_dir,  args.dataset, f'split_{args.split}', args.version)
results_dir = os.path.join(args.result_dir, args.dataset, f'split_{args.split}', args.version)
